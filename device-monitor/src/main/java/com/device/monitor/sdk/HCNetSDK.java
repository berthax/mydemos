...
package com.device.monitor.sdk;
...
public interface HCNetSDK extends StdCallLibrary {
	...
	public static final String SDK_LOCATION = "D:\\dll\\";

//    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(PropUtil.getConfigInfo("dll")+"HCNetSDK", HCNetSDK.class);
    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(SDK_LOCATION + "HCNetSDK", HCNetSDK.class);
...
public static class NET_DVR_FACE_PARAM_CFG extends Structure 
{
	...
	public byte[] byEnableCardReader = new byte[512];  //需要下发人脸的读卡器，按数组表示，从低位到高位表示，0-不下发该读卡器，1-下发到该读卡器
...
public static class NET_DVR_FACE_PARAM_STATUS extends Structure
{
	public int dwSize;
	public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //人脸关联的卡号
	public byte[] byCardReaderRecvStatus = new byte[512];  //人脸读卡器状态，按字节表示，0-失败，1-成功，2-重试或人脸质量差，3-内存已满，4-已存在该人脸，5-非法人脸ID
	public byte[] byErrorMsg = new byte[32]; //下发错误信息，当byCardReaderRecvStatus为4时，表示已存在人脸对应的卡号
...
public static class NET_DVR_FACE_PARAM_COND extends Structure 
{
	public int dwSize;
	public byte[] byCardNo = new byte[ACS_CARD_NO_LEN];    //人脸关联的卡号
	public byte[] byEnableCardReader = new byte[512];  //人脸的读卡器是否有效，0-无效，1-有效
...
//时间点
    ...
    {
    	public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //人脸关联的卡号
    	public byte[] byEnableCardReader = new byte[512];  //人脸的读卡器信息，按数组表示
    	public byte[] byFaceID = new byte[2];        //需要删除的人脸编号，按数组下标，值表示0-不删除，1-删除该人脸
...
public static class RECV_ALARM extends Structure{
    public byte[] RecvBuffer = new byte[2048];//此处的400应不小于最大报警报文长度
}
 ...
 /***API函数声明,详细说明见API手册***/
   ...
   public static interface FMSGCallBack_V30 extends StdCallCallback {
...
//NET_DVR_SetDVRMessage的扩展
 ...
 boolean  NET_DVR_SetDVRMessageCallBack_V30(FMSGCallBack_V30 fMessageCallBack, Pointer pUser);
...
//报警
 ...
 NativeLong  NET_DVR_SetupAlarmChan_V50(NativeLong lUserID);
...
//前面板锁定
 ...
 boolean NET_DVR_GetRemoteConfigState(NativeLong lHandle,Pointer pState);
 boolean NET_DVR_GetDVRConfig(NativeLong lUserID,int dwCommand,NativeLong lChannel,Pointer lpOutBuffer,int dwOutBufferSize,Pointer lpBytesReturned);
 ...
 boolean NET_DVR_ControlGateway(NativeLong lUserID,NativeLong lGatewayIndex,int dwStaic);
 
 NativeLong NET_DVR_StartRemoteConfig(NativeLong lUserID, int dwCommand, Pointer lpInBuffer, int dwInBufferLen, FRemoteConfigCallback fRemoteConfigCallback, Pointer pUserData);

 boolean NET_DVR_RemoteControl(NativeLong lUserID, int dwCommand, Pointer lpInBuffer, int dwInBufferSize);
 /**
	 * 采集人脸信息时的回调函数
	 * @param 入参lpBuffer NET_DVR_CAPTURE_FACE_CFG结构体
	 */
